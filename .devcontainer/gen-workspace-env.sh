...
VERSION="gen-workspace-env.sh version 1.0.1
...
if workspace_is_in_container_volume; then
  ...
  WORKSPACE_CONTAINER_NODE_VOLUME_SOURCE="nodemodules-volume"
  ...
  WORKSPACE_NODE_SOURCE="${WORKSPACE_CONTAINER_NODE_VOLUME_SOURCE}"
  WORKSPACE_NODE_TARGET="${LOCAL_WORKSPACE_FOLDER}/node_modules"
  ...
  WORKSPACE_NODE_SOURCE="${WORKSPACE_SOURCE:?}/node_modules"
  WORKSPACE_NODE_TARGET="${WORKSPACE_ROOT:?}/node_modules"
...
env -i WORKSPACE_CONTAINER_VOLUME_SOURCE="${WORKSPACE_CONTAINER_VOLUME_SOURCE:-}" \
       ...
       WORKSPACE_CONTAINER_NODE_VOLUME_SOURCE="${WORKSPACE_CONTAINER_NODE_VOLUME_SOURCE:-}" \
       ...
       WORKSPACE_NODE_SOURCE="${WORKSPACE_NODE_SOURCE:?}" \
       WORKSPACE_NODE_TARGET="${WORKSPACE_NODE_TARGET:?}" \
...
if [[ $NO_WRITE == false ]]; then
  ...
  # cp ${ENV_FILE_DIR:?}/.env.example "${ENV_FILE_DIR:?}"/.env.d/dev.env
  {
